...
verdicts + calibrated confidence  →  rendered as plain text, no internals
      │
      └── every call is logged to the session entry (audit trail)
```
...
Every API attempt — including transport failures — is appended to the
session via `piRef.appendEntry("ask-jeff", …)` for session-scoped persistence
and audit. The API key is never logged. Config/validation errors before any
request is made are not recorded — only actual API attempts.
